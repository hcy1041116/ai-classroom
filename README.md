...
1. 複製 `backend/.env.example` → `backend/.env`，填入必要金鑰
2. 複製 `frontend/.env.example` → `frontend/.env`
3. 建立 PostgreSQL 資料庫後執行 `cd backend && python seed_data.py`
4. 安裝前端依賴：`cd frontend && npm install`

啟動時需同時開啟三個 Terminal：

```bash
# Terminal 1 — FastAPI 後端（port 8000）
cd backend && .venv/bin/python main.py

# Terminal 2 — LiveKit Agent Worker
cd backend && .venv/bin/python -m agents.voice_pipeline dev

# Terminal 3 — 前端（port 8080）
